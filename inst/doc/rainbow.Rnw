...
By using the \pkg{animate} package \citep{Grahn09}, a set of dynamic movies can be created (see Figure~\ref{fig:9} for an example). The advantage of creating these movies is the ability to demonstrate the time-varying features of the singular components, and to highlight outliers if present in the data. The animation is started by clicking on any graph using Adobe$^{\copyright}$ software.
\begin{figure*}[!ht]
\centering
\scalebox{.523}{\animategraphics{5}{frame_}{286}{342}}\quad
\scalebox{.523}{\animategraphics{5}{frame_}{1}{57}}\quad
\scalebox{.523}{\animategraphics{5}{frame_}{58}{114}}
\\
\scalebox{.523}{\animategraphics{5}{frame_}{115}{171}}\quad
\scalebox{.523}{\animategraphics{5}{frame_}{172}{228}}\quad
\scalebox{.523}{\animategraphics{5}{frame_}{229}{285}}
  \caption{The animation of SVD plot is started by clicking on any graph using Adobe$^{\copyright}$ software.}\label{fig:9}
\end{figure*}
...
The animated SVD plot was produced in a latex editor, such as WinEdt$^{\copyright}$. For creating one animation in Figure~\ref{fig:9}, a series of 57 figures (as there are 57 years in the sea surface temperature data set) are linked together as follows

\begin{smallexample}
  \begin{smallverbatim}
    \% load a package in WinEdt
    \usepackage{animate}
    \% link a series of 57 figures for producing one animation
    \begin{figure}
